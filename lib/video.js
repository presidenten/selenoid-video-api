...
const video = {
  ...
  init(browser, baseUrl) {
    ...
    do {
      browser.pause(100);
    } while(!browser.globals.clientIp);
  },
  ...
  startRecording({ browser, size='1920x1080', framerate=15, codec='libx264', pixelFormat='yuv420p' }) {
    ...
    browser.globals.ffmpeg = spawn('docker', ['exec', recorder.name, 'sh', '-c', ffmpegCMD]);
  },
  ...
  stopRecording({ browser, save=true }) {
    ...
    browser.globals.ffmpeg.stderr.on('data', (data) => {
      if(!data.includes('Exiting normally, received signal 2.')) {
        browser.globals.ffmpeg.kill();
        done = true;
      }
    });
